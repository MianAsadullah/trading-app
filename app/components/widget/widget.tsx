...
export default function Widget({
  ...
  const { settingsShowed, saveSettings, toggleSettings } = useWidgetSettings(
    wid,
    (settings) => {
  ...
  const borderCss = `${
    isMoving
      ? movingCss
      : `border-[1px] border-gray-50 ${tabSettings?.border ?? ""} ${
          tabSettings?.borderColor ?? ""
        }`
  }`;
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden overflow-y-scroll ${borderCss} ${
        cn ?? ""
      }`}
