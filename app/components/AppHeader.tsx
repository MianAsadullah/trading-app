...
export default function AppHeader() {
  ...
  const [tutorialShowed, setTutorialShowed] = useState(
    getLS("tutorialShowed", "")
  );
  const lsUser = getLSUser();
  const isAnonymousUser =
    !lsUser.name || lsUser.name === "User" || lsUser.name === "LOCAL";
    ...
    >
      ...
      <div className="flex gap-6">
        ...
        <Link
          href="/more"
          className={`underlineLink ${
            pathname === "/more" && "underlineLinkActive"
          }`}
        >
          More
        </Link>
      ...
      <div className="flex items-center">
        <Dropdown
          label={isAnonymousUser ? "Profile" : lsUser.name}
          className="text-sm"
          ulClassName="ml-[-40px]"
        >
          <button
