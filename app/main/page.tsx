...
export default function MainPage() {
  ...
  const [userWidgets, setUserWidgets] = useState<UserWidget[]>(
    getLS(`userWidgets${tab}`, DefaultWidgets, true)
  );
  const [layout, setLayout] = useState<Layout[]>(
    getLS(`userLayout${tab}`, DefaultLayout, true)
  );
  ...
  useSub(PubSubEvent.Delete, async (wid: string) => {
    if (confirm("Delete this widget?") === true) {
      ...
      setUserWidgets((userWidgets: UserWidget[]) =>
    ...
    <>
      ...
      >
        <ResponsiveGridLayout
          ...
          layouts={layouts}
          onBreakpointChange={(newBreakpoint: any, newCols: any) => {
      ...
      <footer className="ml-4 mt-2 text-sm">
        Dashb.io -{" "}
        <Link href="/more" className="link-minor">
          Dashboard Examples
        </Link>
        {" - "}
        <a
          href="https://github.com/ngduc/dashb"
          target="_blank"
        ...
        {" - "}
        <Link href="/terms-of-service" className="link-minor">
          Terms
        </Link>
        {" - "}
        <span>
