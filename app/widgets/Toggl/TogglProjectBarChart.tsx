      ...
      <div className="custom-tooltip">
        ...
        {payload.map((item: any) => (
          <p key={item.name} style={{ color: item.color }}>{`${
            item.name
          }: ${item.value.toFixed(2)} minutes`}</p>
        ))}
      </div>
    );
  }
...
export default function TogglProjectBarChart({ wid }: Props) {
  ...
  const fetchData = async () => {
    ...
    filteredEntries.forEach((entry: any) => {
      ...
      // entry.tzStart = format(
      //   zonedTimeToUtc(entry.start, pacificTimeZone),
      //   "MM-dd"
      // );
      // entry.tzStop = format(
      //   zonedTimeToUtc(entry.stop, pacificTimeZone),
      //   "MM-dd"
      // );
    ...
    Object.keys(groupedData).forEach((date) => {
      ...
      dateGroup.forEach((entry: any) => {
        item[entry.project_name + "_pct"] = Math.round(
          (item[entry.project_name] * 100) / item.total
        );
      });
