...
export default function Note({ wid }: Props) {
  ...
  const { settings, setSettings, saveSettings } = useWidgetSettings(
    wid,
    (settings) => {
    ...
    <Widget
      ...
      render={() => {
        ...
        return (
          ...
          >
            {isSaving && (
              <span className="absolute right-2 top-2 w-2 h-2 bg-green-300 rounded-full"></span>
            )}
            <textarea
              ...
              {arrNotes.map((item, idx) => {
                ...
                return (
                  <li
                    key={idx}
                    data-idx={idx}
                    className={`px-2 py-1 cursor-pointer hover:text-gray-500 ${
                      noteIndex === idx && "bg-gray-100"
                    }`}
                    onClick={(ev: any) => {
                      if (!isSaving) {
